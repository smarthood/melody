...
export function activate(context: vscode.ExtensionContext) {
  ...
  async function handleCommand(isplay: boolean) {
    if (!isplay) {
      ...
      song = songsDisplay[Math.floor(Math.random() * songsDetail.length)];
    }
  ...
  function updateWebviewContent(panel: vscode.WebviewPanel, selectedSong: any) {
    ...
    const htmlContent = getHtmlContent(audioElement);
  ...
  function getHtmlContent(audioElement: string): string {
    return `
      <!DOCTYPE html>
      <html lang="en">
      <head>
